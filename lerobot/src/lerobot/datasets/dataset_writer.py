...
class DatasetWriter:
    ...
    def add_frame(self, frame: dict) -> None:
        ...
        # 中文注释：这是理想的等间隔逻辑时间，不是墙上时钟；循环偶尔变慢不会反映在此列中。
    ...
    def save_episode(
        ...
        # 中文注释：frame_index 每个 episode 归零，index 则在整个数据集内连续递增；
        # meta/episodes 中的 [dataset_from_index, dataset_to_index) 用它来定位 episode。
    ...
    def _save_episode_video(
        ...
        # 中文注释：多个 episode 可以拼进同一个 MP4；这对时间边界告诉读取器该解码哪一段。
        # 因此不能假设“一个 MP4 文件就是一个 episode”。
        return metadata
